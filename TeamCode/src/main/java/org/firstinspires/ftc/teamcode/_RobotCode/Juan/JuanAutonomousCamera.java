...
import org.firstinspires.ftc.teamcode._RobotCode.Juan.JuanPayload;
import org.firstinspires.ftc.teamcode._RobotCode.Juan_RELEASED.Juan_RELEASED;

@Autonomous(name = "JUAN AUTO CAMERA", group="JUAN")
@Config
...
public class JuanAutonomousCamera extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        SleeveColor coneSide = payload.getScanner().scan();
...
//        lift.goToPreset(JuanPayload.LiftHeight.CRUISE);

        // Turn to High Pole
        goToPose(47, -1, -42, .7);

        // Wait for Lift
        while ( lift.isDoneMoving()) {
            robot.update();
            telemetry.update();
        }
        ...
        // Lower Gripper
        lift.goToPreset(JuanPayload.LiftHeight.STACK_1);
...
//        goToPose( 47, 20, 90, .7);
        ...
        // GREEN
        if(coneSide==SleeveColor.GREEN) {
            goToPose(23, -22, 180, 1);
        }
        // PURPLE
        else if(coneSide==SleeveColor.PURPLE){
        // ORANGE
            // Do Nothing...
        }
        else{
